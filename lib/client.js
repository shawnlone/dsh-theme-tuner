// dsh-theme-customizer — CLIENT half (browser bundle)
//
// This is the deployable artifact the module loader fetches at
// /plugins/dsh-theme-customizer/client.js. It registers a row in the General
...
// persists both palettes through the "theme-customizer" settings namespace, and
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var NAMESPACE = "theme-customizer";
    /** Locale keyspace bound to the section's t(). */
    var LOCALE_NS = "settings.themeCustomizer";
    ...
    var cssText = [
      ...
      "." + cssClass.row + "{" +
        ...
        "padding:14px 0;border-bottom:1px solid var(--dsw-alias-border-l1);" +
      "}",
      "." + cssClass.rowLabel + "{min-width:76px;font-size:13px;color:var(--dsw-alias-label-secondary);}",
      "." + cssClass.swatch + "{" +
        "position:relative;display:inline-flex;align-items:center;gap:10px;min-width:176px;padding:5px 6px;" +
        "border-radius:9px;background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);" +
        "cursor:pointer;transition:border-color .15s;" +
      "}",
      "." + cssClass.swatch + ":hover{border-color:var(--dsw-alias-border-l2);}",
      "." + cssClass.swatchDot + "{" +
        "width:22px;height:22px;border-radius:50%;flex:none;border:1px solid var(--dsw-alias-border-l2);" +
      "}",
      "." + cssClass.swatchHex + "{font:400 12.5px var(--ds-font-family-code);color:var(--dsw-alias-label-primary);}",
    ...
    function installStyles(ctx) {
      ...
      ctx.effect(function () {
        ...
        tag.dataset.plugin = "dsh-theme-customizer";
        tag.dataset.pluginCss = "dsh-theme-customizer/styles";
     ...
     */
    function ThemeCustomizerRow(props) {
      ...
      children.push(h("div", { className: cssClass.header },
        h("div", { className: cssClass.title }, t("title")),
      ...
      colorRows.forEach(function (r) {
        ...
        children.push(h("div", { className: cssClass.row, key: r.key },
          h("span", { className: cssClass.rowLabel }, r.label),
    ...
    function apply(ctx) {
      ...
      function applyTokens() {
        ...
        ctx.theme.overrideTokens("theme-customizer", deriveTokens(currentConfig()));
      }

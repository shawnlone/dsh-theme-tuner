...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var DEFAULTS = {
      light: { accent: "#339cff", bg: "#ffffff", fg: "#1a1c1f", contrast: 45, opacity: 0 },
      dark: { accent: "#2d99ff", bg: "#262626", fg: "#d1d1d1", contrast: 28, opacity: 0 },
    };
    ...
    /** Wrap a hex color in rgba() with the given alpha (translucency helper). */
    ...
    function deriveScheme(settings, scheme) {
      ...
      // Overall translucency (0 = opaque / default). Higher opacity value => more
      // transparent surfaces; alpha is banded (min 0.3) so the UI never fully
      // disappears even at 100.
      var opacity = settings.opacity || 0;
      var surfAlpha = clamp(1 - opacity / 100, 0.3, 1);
      function surf(col) {
        return opacity > 0 ? withAlpha(col, surfAlpha) : col;
      }
      return {
        "--dsw-alias-bg-base": surf(bg),
        "--dsw-alias-bg-layer-1": surf(layer1),
        "--dsw-alias-bg-layer-2": surf(layer2),
        "--dsw-alias-bg-layer-3": surf(layer3),
        "--dsw-specific-sidebar-fill": surf(layer1),
    ...
    function ThemeTunerRow(props) {
      ...
      var opacity = config.opacity || 0;
      children.push(h("div", { className: cssClass.row, key: "opacity" },
        h("span", { className: cssClass.rowLabel }, t("opacity")),
        h("div", { className: cssClass.sliderWrap },
          h("input", {
            className: cssClass.slider,
            type: "range",
            ...
            value: opacity,
            style: { "--fill": opacity + "%" },
            onChange: function (e) { setField(mode, "opacity", Number(e.target.value)); },
          }),
          h("span", { className: cssClass.sliderVal }, String(opacity))
        )
      ));

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function deriveScheme(settings, scheme) {
      ...
      var top = bg, bottom = bg;
      if (g !== 0) {
        var shift = 0.5 * clamp(Math.abs(g) / 100, 0, 1); // max 50% toward white/black
        var lightTint = mixHsl(bg, "#FFFFFF", shift);
        var darkTint = mixHsl(bg, "#000000", shift);
        if (g > 0) { top = lightTint; bottom = darkTint; }
        else { top = darkTint; bottom = lightTint; }
      }
      var bgBase = g !== 0 ? "linear-gradient(180deg," + top + " 0%," + bg + " 50%," + bottom + " 100%)" : bg;
      return {

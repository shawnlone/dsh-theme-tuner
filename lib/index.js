...
function schemeSchema(defaults) {
  return z.object({
    ...
    opacity: z.number().min(0).max(100).default(defaults.opacity),
  });
}

/** Durable schema: two top-level fields (`light` / `dark`), each an object. */
const CustomizerSchema = z.object({
  light: schemeSchema({ accent: "#339cff", bg: "#ffffff", fg: "#1a1c1f", contrast: 45, opacity: 0 }),
  dark: schemeSchema({ accent: "#2d99ff", bg: "#262626", fg: "#d1d1d1", contrast: 28, opacity: 0 }),
});
